...
import { AwardAbi2, PrizePoolAbi, PrizePoolAbi2, SOX } from "src/typechain";
...
export const changeApproval = createAsyncThunk(
  ...
  async ({ token, provider, address, networkID }: IChangeApprovalAsyncThunk, { dispatch }) => {
    ...
    const soxContract = new ethers.Contract(addresses[networkID].SOX_ADDRESS, ierc20Abi, signer) as SOX;

    let approveTx;
    let depositAllowance = await soxContract.allowance(address, addresses[networkID].PT_PRIZE_POOL_ADDRESS);
    ...
    depositAllowance = await soxContract.allowance(address, addresses[networkID].PT_PRIZE_POOL_ADDRESS);
