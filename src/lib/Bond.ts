...
export class LPBond extends Bond {
  ...
  async getTreasuryBalance(networkID: NetworkID, provider: StaticJsonRpcProvider) {
   ...
   // console.error(`lp price ---------${tokenAmount}------${token.address}------${addresses[networkID].TREASURY_ADDRESS}---------${tokenAddress}-----${bondCalculator.address}---------${token.address}-${valuation}---${markdown}---------`)
    return tokenUSD;
  }
}
...
export class StableBond extends Bond {
  ...
  async getTreasuryBalance(networkID: NetworkID, provider: StaticJsonRpcProvider) {
   ...
   // console.error(`Stable price ---------${tokenAmount}------${token.address}------${addresses[networkID].TREASURY_ADDRESS}------------${token.address}------------`)
