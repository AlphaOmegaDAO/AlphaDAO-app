...
import { OXBUSD_LPBOND_NAME, PCS_OXBUSD_PAIR } from "./utils/Constants";
...
export function handleBondDeposit(call: DepositCall): void {
  ...
  let token = loadOrCreateToken(OXBUSD_LPBOND_NAME);
  ...
  deposit.value = getPairUSD(call.inputs.amount_, PCS_OXBUSD_PAIR);
...
export function handleBondRedeem(call: RedeemCall): void {
  ...
  redemption.token = loadOrCreateToken(OXBUSD_LPBOND_NAME).id;
