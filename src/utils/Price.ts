import { PCS_OXUSDT_PAIR } from "./Constants";
...
export function getOxUSDRate(): BigDecimal {
  let pair = UniswapV2Pair.bind(Address.fromString(PCS_OXUSDT_PAIR));

  let reserves = pair.getReserves();
  let reserve0 = reserves.value0.toBigDecimal();
  let reserve1 = reserves.value1.toBigDecimal();

  let oxRate = reserve1.div(reserve0).div(BIG_DECIMAL_1E9);
  log.debug("OX rate {}", [oxRate.toString()]);

  return oxRate;
}

//(slp_treasury/slp_supply)*(2*sqrt(lp_usdt * lp_ox))
...
export function getPairUSD(lp_amount: BigInt, pair_adress: string): BigDecimal {
  ...
  let ox_value = toDecimal(lp_token_0, 9).times(getOxUSDRate());
  let total_lp_usd = ox_value.plus(toDecimal(lp_token_1, 18));
