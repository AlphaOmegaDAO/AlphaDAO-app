...
export function loadOrCreateProtocolMetric(timestamp: BigInt): ProtocolMetric {
  ...
  if (protocolMetric == null) {
    ...
    protocolMetric.xOxCirculatingSupply = BigDecimal.fromString("0");
...
function getSOxSupply(): BigDecimal {
  let xOx_supply = BigDecimal.fromString("0");

  let xOx_contract = xOxERC20.bind(
    Address.fromString(SOX_ERC20_CONTRACT)
  );
  xOx_supply = toDecimal(xOx_contract.circulatingSupply(), 9);

  log.debug("xOx Supply {}", [xOx_supply.toString()]);
  return xOx_supply;
}
...
function getRunway(
  ...
  if (
    xOx.gt(BigDecimal.fromString("0")) &&
    rfv.gt(BigDecimal.fromString("0")) &&
    rebase.gt(BigDecimal.fromString("0"))
  ) {
    let treasury_runway = Number.parseFloat(rfv.div(xOx).toString());
...
export function updateProtocolMetrics(transaction: Transaction): void {
  ...
  //xOx Supply
  pm.xOxCirculatingSupply = getSOxSupply();
  ...
  //Total Value Locked
  pm.totalValueLocked = pm.xOxCirculatingSupply.times(pm.oxPrice);
